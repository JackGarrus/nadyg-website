...
Originally, Kubernetes only worked with Docker, but over time, people wanted to use different runtimes (like containerd standalone, etc.).
...
At that point, Docker Inc. realized something important: “We're not going to make money from dockerd or containerd, but we can make money from developer tools!”
...
<br />
Now the ecosystem could adopt containerd freely, and Docker could monetize elsewhere.
